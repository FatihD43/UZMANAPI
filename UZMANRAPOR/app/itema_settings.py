...
from typing import Dict, List, Optional, Protocol, Iterable, Any, Sequence
...
class CursorLike(Protocol):
    ...
    # ApiCursor.execute(query, params=[...]) tarzını desteklemek için:
...
# ---------------------------------------------------------------------------
# 4) Stored procedure çağrıları
# ---------------------------------------------------------------------------

def get_itema_automatic_settings(conn: ConnectionLike, tip: str) -> Optional[Dict[str, Optional[str]]]:
    cur = conn.cursor()
    # NOT: API katmanı named param (@Tip=) yerine pozisyonel parametreyi daha sağlam taşır.
